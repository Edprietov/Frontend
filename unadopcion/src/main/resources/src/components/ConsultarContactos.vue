<template>
  <div card>
    ...
    <div class="form-row" id="caja">
      <label class="control-label col-sm-2" for="nombreContactoFiltro"
        ><strong>Nombre Contacto a Consultar:</strong></label
      >
      <div class="col-sm-0">
        <input
          required
   ...
   <label class="control-label col-sm-1" for="ciudadFiltro"
      ...
      <div class="col-sm-1">
        <input  required
          type="text"
         id="telefonoFiltro" class="form-control" v-model="telefonoFiltro" />
      </div>

      <button class="btn btn-success" id="botonFiltro" @click="filtrar">Filtrar</button>
    </div>
    <table class="table" aria-describedby="consultarM">
      ...
      <tbody>
        ...
        <td>{{ valor[0] }}</td>
          ...
          <td>{{(valor[5] !== null)? valor[5]  :  "-" }}</td>
          <td>{{ (valor[6] !== null)? valor[6] : "-" }}</td>
          <td>{{ (valor[3] !== null)? valor[3] : "-" }}</td>
          <td>{{ (valor[4] !== null)? valor[4] : "-" }}</td>
        </tr>
      </tbody>
    </table>
...
export default {
  ...
  this.ConsultarContactos();
    ...
    filtrar(e) {
      e.preventDefault();
      if( this.nombreContactoFiltro == "" && this.ciudadFiltro == "" && this.correoFiltro == "" && this.telefonoFiltro == ""){
      ...
      for(let x in datos){
           ...
           datos[x][6] = (datos[x][6] == null)? "-" : datos[x][6];
           if(datos[x][1].includes(this.nombreContactoFiltro) && datos[x][3].includes(this.ciudadFiltro) &&  datos[x][5].includes(this.correoFiltro)  && datos[x][6].includes(this.telefonoFiltro)){
